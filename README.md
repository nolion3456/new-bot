...
执行 `/audit-channel` 后，拥有“管理服务器”权限的成员可以在私密面板中通过按钮开启或关闭日志类型，并管理最多 3 个后台审计频道。按钮会以 `✅` 或 `⬜` 显示当前状态，可选日志类型包括身份组变动、昵称变动、消息编辑、消息删除、成员加入和成员离开。关闭某项后，该项事件不会发送到后台频道；相同的已启用日志会同步发送到全部已配置频道。
